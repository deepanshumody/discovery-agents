...
python -m discovery_agents.mlinfra.cli bench           # Numpy vs Zarr vs HDF5 read throughput
```

Granular commands: `curate`, `train`, `bench`, `export` (also `make -f deploy/Makefile <target>`).
